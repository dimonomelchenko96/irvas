...
window.addEventListener('DOMContentLoaded', () => {
  ...
  let deadline = new Date(Date.parse(new Date()) + 15 * 24 * 60 * 60 * 1000); // let deadline = '2022-01-20';
...
const images = () => {
  ...
  bigImage.style.cssText = 'width: auto; max-width: 80%; height: 80vh; object-fit: cover';
...
const modals = state => {
  function bindModal(triggerSelector, modalSelector, closeSelector, closeClickOverlay = true) {
    const trigger = document.querySelectorAll(triggerSelector),
          ...
          scroll = calcScroll(); // открытие модалки
    ...
    trigger.forEach(item => {
      item.addEventListener('click', e => {
        ...
        if (target.classList.contains('popup_calc_button') && (parseInt(state.width) <= 500 || state.width === '' || parseInt(state.height) <= 500 || state.height === '' ? true : false)) {
...
const timer = (id, deadline) => {
  ...
  console.log(getTimeRemaining('2022-01-20')); // установка таймера на страницу
